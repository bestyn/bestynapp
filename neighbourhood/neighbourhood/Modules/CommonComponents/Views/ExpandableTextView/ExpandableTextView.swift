...
@objc protocol ExpandableTextViewDelegate: UITextViewDelegate {
    func expandableTextView(_ textView: ExpandableTextView, didChangeState isExpanded: Bool)
}

@IBDesignable
class ExpandableTextView: UITextView {
    public weak var expandableDelegate: ExpandableTextViewDelegate?
...
//    public var onSizeChange: (                                               )->() = { _ in }
    ...
    */
    ...
    private func invokeOnSizeChangeIfNeeded() {
        if let cachedIntrinsicContentHeight = cachedIntrinsicContentHeight {
            if intrinsicContentHeight != cachedIntrinsicContentHeight {
                ...
                expandableDelegate?.expandableTextView(self, didChangeState: state == .expanded)
            }
        } else {
            self.cachedIntrinsicContentHeight = intrinsicContentHeight
            expandableDelegate?.expandableTextView(self, didChangeState: state == .expanded)
    ...
    private func rangeToReplaceWithReadMoreText() -> NSRange {
        print("SHOW LESS", layoutManager.characterRangeThatFits(textContainer: textContainer), text)
